        ...
        for (int c = 0; c < OUTPUT_C; c++)
            for (int h = 0; h < OUTPUT_H; h++)
                for (int w = 0; w < OUTPUT_W; w++)
                {
                    ...
                    //std::cout << "pos: " << h << ' ' << w << ' ' << c << std::endl;
                    //std::cout << "pixel is: " << fp16::__half2float(output[posCHW]) << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    auto fileName = locateFile("test_interleaving.uff");
    ...
    */
    ...
    processImage("bus_320x240.ppm", "poop.ppm", context, args.fp16);
